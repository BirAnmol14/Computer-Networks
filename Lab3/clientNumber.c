...
int main(){
	...
	double d;scanf("%lf",&d);
	int byteSent = send(sock,&d,sizeof(d),0);//0 is flag for default behaviour
	if(byteSent!=sizeof(d)){
	...
	//5. Receive bytes
	int x;
	int byteRecv = recv(sock,&x,sizeof(x),0);//buff1 should be the address of your buffer, here array so direct name else & name
	if(byteRecv<0){
		puts("Receive Error");
		exit(0);
	}
