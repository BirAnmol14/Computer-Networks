...
int main(){
	...
	sendto(sock,buff_cmd,2,0,(struct sockaddr *)&server,sizeof(server));//send to server the command
	if(command ==1 || command ==2){
		...
		sendto(sock,buff_offset,10,0,(struct sockaddr *)&server,sizeof(server));//send and write to server the file offset
	}	
	else{
	...
	int servlen;
	while((bytesRecv=recvfrom(sock,recvBuf,BUF,0,(struct sockaddr *)&server,&servlen))>0){
