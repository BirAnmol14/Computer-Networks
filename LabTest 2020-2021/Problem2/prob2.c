...
ID: 2018A7PS0261P
CMD LINE ARGS: ./a.out <MY PORT> <PEER PORT>
*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char * argv[]){
	if(argc<3||argc>3){
		die("Inavalid argument use: ./a.out <PORT> <PEER PORT>");
	}
	int port = atoi(argv[1]);
	int peer = atoi(argv[2]);
	//SET ALARM HANDLER
	...
	if(pid==0){
		...
		while(!done && (n=fscanf(fp,"%[^\n]",curr.payload))!=EOF){
			...
			Packet rcv;
			int r = recvfrom(sock,&rcv,sizeof(rcv),0,(struct sockaddr *)&other,&len);
			if(r<0){
					if(errno == EINTR){
